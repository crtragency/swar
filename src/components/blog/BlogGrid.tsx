            ...
            <Link href={`/blog/${post.slug}`} className="block h-full">
              ...
              >
                <div className="relative h-56 overflow-hidden">
                  <Image src={post.image} alt={postText(locale, post, "title")} fill sizes="(max-width:768px) 100vw, 33vw" className="object-cover transition-transform duration-[1200ms] group-hover:scale-110" />
                  <span className="absolute right-4 top-4 rounded-full bg-navy-950/70 px-4 py-1.5 text-xs font-bold text-gold-400 backdrop-blur-md">{postText(locale, post, "category")}</span>
                </div>
                <div className="flex flex-1 flex-col p-7">
                  <time className="text-sm font-medium text-navy-900/50">{post.date}</time>
                  <h3 className="mt-3 text-xl font-extrabold leading-snug text-navy-900 transition-colors group-hover:text-ocean-600">{postText(locale, post, "title")}</h3>
                  <p className="mt-3 flex-1 leading-relaxed text-navy-900/65">{postText(locale, post, "excerpt")}</p>
